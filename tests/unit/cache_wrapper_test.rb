...
module AwsDevUtils
  ...
  class CacheWrapperTester < TestCase
    ...
    def test_with_cache_set_value_without_args
      ...
      cache.set(["test-client", :mock_func, {}, 60], value, 60)
      assert_equal value, client.mock_func
      assert_equal value, cache.get(["test-client", :mock_func, {}, 60])
    end

    def test_with_cache_set_value_with_args
      client = client_setup
      value = {res: 'test-value-from-cache'}
      cache.set(["test-client", :mock_func, {filters: [{name: 'filter1', value: ['val1', 'val2']}]}, 60], value, 60)
      assert_equal value, client.mock_func({filters: [{name: 'filter1', value: ['val1', 'val2']}]})
      assert_equal value, cache.get(["test-client", :mock_func, {filters: [{name: 'filter1', value: ['val1', 'val2']}]}, 60])
    end
    ...
    def test_sort_filters
      ...
      cache.set(["test-client", :mock_func, filters.deep_sort, 60], value, 60)
      assert_equal value, client.mock_func(filters)
      assert_equal value, cache.get(["test-client", :mock_func, filters.deep_sort, 60])
      assert_nil cache.get(["test-client", :mock_func, filters, 60])
    end
    ...
    def client_setup
      CacheWrapper.new MockClient.new, client_name: "test-client"
    end
